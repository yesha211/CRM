...
exports.DeleteCustomer = async (req, res, next) => {
    ...
    try {
        ...
        await pool.query(
            `DELETE FROM MCustomer  WHERE sCustomerGUID = ? `,
            [sCustomerGUID],
        );
    } catch (err) {
        const error = new ErrorHandler(
...
exports.UpdateCustomer = async (req, res, next) => {
    const { bInActive, sAddress, sCustomerName, sEmail, sLanguage, sMobileNumber, sSocialAccounts, sCustomerGUID } = req.body;
    const pool = req.pool;
    let address;
    let language;
    let socialAccounts;
    ...
    try {
        ...
        await pool.query(
            ...
            [address, sCustomerName, socialAccounts, language, sMobileNumber, bInActive, sEmail, sEmail],
        );
        const [rows] = await pool.query(
...
exports.UpdateCustomerByParams = async (req, res, next) => {
    const { sAddress, sCustomerName, sEmail, sLanguage, sMobileNumber, sSocialAccounts,bInActive } = req.body;
    const { sCustomerGUID } = req.params;
    const pool = req.pool;
    let address;
    let language;
    ...
    try {
        ...
        await pool.query(
            ...
            [address, sEmail, socialAccounts, sMobileNumber,bInActive, sCustomerName, language, sCustomerGUID],
        );
        const [rows] = await pool.query(
            `SELECT * FROM MCustomer WHERE sCustomerGUID = ? `,
            [sCustomerGUID]
