...
} export async function apigetTemplateFile<T, U extends Record<string, unknown>>(data: U) {
    return ApiService.fetchData<T>({
        url: `/gettemplatefile`,
        method: 'get',
        data
...
} export async function apisetTemplateInActive<T, U extends Record<string, unknown>>(data: U) {
    return ApiService.fetchData<T>({
